...
function obterIdSemanaAtual() {

    const semana = obterSemanaAtual();

    return [
        semana.segunda.getFullYear(),
        String(semana.segunda.getMonth() + 1).padStart(2, "0"),
        String(semana.segunda.getDate()).padStart(2, "0")
    ].join("-");
}


// --------------------------------------------
// SALVA A SEMANA NO HISTÓRICO
// --------------------------------------------

function salvarSemanaNoHistorico() {
    ...
    const historicoSalvo =
        JSON.parse(
            localStorage.getItem("historicoSemanas") || "[]"
        );

    const idSemana = obterIdSemanaAtual();

    // Evita salvar a mesma semana duas vezes
    const jaExiste = historicoSalvo.some(function(semanaSalva) {

        return semanaSalva.id === idSemana;
    ...
    if (!jaExiste) {
        ...
        historicoSalvo.push({
            ...
            inicio: formatarDataHistorico(
                semana.segunda
            ),

            fim: formatarDataHistorico(
                semana.sexta
            ),
...
// --------------------------------------------
// ZERA AS MARCAÇÕES DA SEMANA
// --------------------------------------------

function zerarMarcacoesSemana() {
...
function prepararFinalizacaoSemana() {
    ...
    const semanaJaFinalizada =
        localStorage.getItem("semanaFinalizada");

    // Impede finalizar novamente a mesma semana
    if (semanaJaFinalizada === idSemana) {

        alert(
            "Esta semana já foi finalizada.\n\n" +
            "O Planner está aguardando a próxima semana."
        );
    ...
    const confirmar = confirm(
        ...
        "e as marcações das tarefas serão zeradas " +
    ...
    // 1 — Salva a semana
    salvarSemanaNoHistorico();


    // 2 — Marca a semana como finalizada
    localStorage.setItem(
        "semanaFinalizada",
        idSemana
    );
    ...
    // 3 — Zera somente os checkboxes
    zerarMarcacoesSemana();
    ...
    alert(
        ...
        "A semana foi salva no Histórico.\n\n" +

        "As tarefas continuam no Planner, " +
        "mas as marcações foram zeradas.\n\n" +

        "Na próxima semana o Planner poderá " +
        "começar um novo ciclo."

    );


    // 5 — Atualiza a tela do Histórico
...
function atualizarStatusFinalizacaoSemana() {

    const status =
        document.querySelector(".historico-status");

    const botao =
        document.querySelector(".btn-finalizar-semana");
    ...
    const semanaFinalizada =
        localStorage.getItem("semanaFinalizada");


    if (semanaFinalizada === idSemana) {

        status.innerHTML =
            "✅ Semana finalizada e salva no Histórico.";

        botao.textContent =
            "✅ Semana já finalizada";
...
// ============================================
// ATUALIZAÇÃO COMPLETA DO HISTÓRICO
// ============================================

const atualizarHistoricoOriginal =
    atualizarHistorico;
...
atualizarHistorico = function() {
    ...
    atualizarStatusFinalizacaoSemana();
